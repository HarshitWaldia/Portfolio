  ...
  return (
    <>
      ...
      >
        ...
        <div className="absolute bottom-4 left-0 right-0 flex flex-col items-center justify-center pointer-events-none z-40">
...
const CoverFlowItemCard = memo(function CoverFlowItemCard({
  ...
  const filterStyle = useTransform(
    scrollX,
    (value) => `brightness(${Math.abs(index - value) < 0.5 ? 1 : 0.4})`,
  )
  ...
  return (
    <motion.div
      className={`absolute top-1/2 left-1/2 will-change-transform group-[.is-dragging]/cf:!cursor-grabbing ${cursorClass}`}
      style={{
        width,
        height,
        marginTop: -height / 2,
